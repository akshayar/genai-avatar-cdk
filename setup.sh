...
API_OUTPUT=$(aws cloudformation describe-stacks --region us-east-1 --stack-name $BACKEND_STACK_NAME --query "Stacks[0].Outputs[?OutputKey=='$OUTPUT_KEY'].OutputValue" --output text)
OUTPUT_KEY="identityPoolOutput"
IDENTITY_OUTPUT=$(aws cloudformation describe-stacks --region us-east-1 --stack-name $BACKEND_STACK_NAME --query "Stacks[0].Outputs[?OutputKey=='$OUTPUT_KEY'].OutputValue" --output text)
echo $API_OUTPUT
...
OUTPUT_VALUE=$(aws cloudformation describe-stacks --region us-east-1 --stack-name $FRONTEND_STACK_NAME --query "Stacks[0].Outputs[?OutputKey=='$OUTPUT_KEY'].OutputValue" --output text)
